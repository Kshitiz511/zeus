      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 overflow-y-auto bg-white px-6 pb-8 pt-24 md:hidden"
          >
            <nav className="space-y-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="flex h-12 items-center text-[16px] font-medium text-[var(--zeus-black)]"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <Link
              href="/book-a-call"
              className="hidden md:inline-flex h-10 items-center justify-start rounded-lg bg-[var(--zeus-blue)] px-4 pr-4 text-[14px] font-semibold text-white transition-opacity hover:opacity-90"
            >
              <span className="ml-[1px] inline-flex items-center gap-2">
                Book a Call
                <ArrowRight size={14} />
              </span>
            </Link>
